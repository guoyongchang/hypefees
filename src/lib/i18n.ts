...
const translations: Record<string, Record<Lang, string>> = {
  ...
  'switch.walletWarningDesc': e('If you keep using a wallet that charges builder fees (like {wallet}), you\'ll still pay their fee on every trade. Switch to a 0% wallet like OneKey to stop paying builder fees permanently.','如果你继续使用收取 Builder Fee 的钱包（如 {wallet}），每笔交易仍然会被收费。建议切换到 OneKey 等 0% 费率钱包，从根本上杜绝 Builder Fee。','ビルダー手数料を請求するウォレット（{wallet}など）を使い続けると、毎回手数料がかかります。OneKeyなどの0%ウォレットに切り替えましょう。','빌더 수수료를 부과하는 지갑({wallet})을 계속 사용하면 매 거래마다 수수료가 발생합니다. OneKey 같은 0% 지갑으로 전환하세요.','Si sigues usando una billetera que cobra builder fees (como {wallet}), seguirás pagando. Cambia a una billetera 0% como OneKey.','Si vous continuez avec un wallet qui facture des frais builder (comme {wallet}), vous paierez toujours. Passez à un wallet 0% comme OneKey.','Wenn du weiterhin eine Wallet mit Builder-Gebühren ({wallet}) nutzt, zahlst du weiter. Wechsle zu einer 0%-Wallet wie OneKey.','Se continuar usando uma carteira que cobra builder fees (como {wallet}), continuará pagando. Mude para uma carteira 0% como a OneKey.','Если продолжите использовать кошелёк с комиссией ({wallet}), вы всё равно будете платить. Перейдите на кошелёк с 0% как OneKey.','Builder ücreti alan bir cüzdan ({wallet}) kullanmaya devam ederseniz ücret ödemeye devam edersiniz. OneKey gibi %0 cüzdana geçin.'),
