...
const translations: Record<string, Record<Lang, string>> = {
  ...
  'hero.savedCounter': e('{users} traders saved {savings} with HypeFees','{users} 位交易者通过 HypeFees 节省了 {savings}','{users} 人のトレーダーが HypeFees で {savings} を節約','{users}명의 트레이더가 HypeFees로 {savings} 절약','{users} traders ahorraron {savings} con HypeFees','{users} traders ont économisé {savings} avec HypeFees','{users} Trader haben {savings} mit HypeFees gespart','{users} traders economizaram {savings} com HypeFees','{users} трейдеров сэкономили {savings} с HypeFees','{users} trader HypeFees ile {savings} tasarruf etti'),
  ...
  'result.currentBuilder': e('Your Current Builder Fee','你当前的 Builder 费率','現在のビルダー手数料率','현재 빌더 수수료율','Tu Comisión Builder Actual','Votre Frais Builder Actuel','Deine Aktuelle Builder-Gebühr','Sua Taxa Builder Atual','Ваша Текущая Комиссия Билдера','Mevcut Builder Ücretiniz'),
  'result.builderRate': e('~{rate}% per trade via your wallet','每笔交易约 {rate}%（来自你的钱包）','取引あたり約{rate}%（ウォレット経由）','거래당 약 {rate}% (지갑 경유)','~{rate}% por operación vía tu billetera','~{rate}% par trade via votre wallet','~{rate}% pro Trade über deine Wallet','~{rate}% por trade via sua carteira','~{rate}% за сделку через ваш кошелёк','İşlem başına ~%{rate} cüzdan aracılığıyla'),
  'result.couldSaveMonthly': e('Estimated ~${amount}/month if you switch to 0%','切换到 0% 预计每月可省 ~${amount}','0%に切り替えると毎月約${amount}節約','0%로 전환하면 월 약 ${amount} 절약','Estimación ~${amount}/mes al cambiar a 0%','Estimation ~${amount}/mois en passant à 0%','Geschätzte ~${amount}/Monat bei Wechsel auf 0%','Estimativa ~${amount}/mês ao mudar para 0%','Экономия ~${amount}/мес при переходе на 0%','0%\'a geçerseniz tahmini aylık ~${amount} tasarruf'),

  // ── Builder Status (unified card) ──
  'result.nowOnOneKey': e('You\'re on OneKey', '你已绑定 OneKey', 'OneKeyに設定済み', 'OneKey에 설정됨', 'Estás en OneKey', 'Vous êtes sur OneKey', 'Du bist auf OneKey', 'Você está na OneKey', 'Вы на OneKey', 'OneKey\'desiniz'),
  'result.currentFeeIs0': e('Your builder fee is now 0%', '你的 Builder Fee 现在是 0%', 'ビルダー手数料は現在0%です', '빌더 수수료가 현재 0%입니다', 'Tu comisión builder es ahora 0%', 'Vos frais builder sont maintenant 0%', 'Deine Builder-Gebühr ist jetzt 0%', 'Sua taxa builder é agora 0%', 'Ваша комиссия билдера теперь 0%', 'Builder ücretiniz artık %0'),
  'result.builderFeeNow': e('builder fee', 'Builder Fee', 'ビルダー手数料', '빌더 수수료', 'comisión builder', 'frais builder', 'Builder-Gebühr', 'taxa builder', 'комиссия', 'builder ücreti'),
  'result.historicalAvg': e('Historical average (past trades)', '历史平均（过往交易）', '過去の平均（過去の取引）', '과거 평균 (지난 거래)', 'Promedio histórico (trades pasados)', 'Moyenne historique (trades passés)', 'Historischer Durchschnitt (vergangene Trades)', 'Média histórica (trades passados)', 'Историческое среднее (прошлые сделки)', 'Geçmiş ortalama (önceki işlemler)'),
  'result.historicalNote': e('You paid {amount} in builder fees before switching. Future trades through OneKey or other 0% wallets will incur no builder fees.', '切换前你已支付 {amount} Builder Fee。今后通过 OneKey 或其他零费率钱包交易将不再产生 Builder Fee。', '切り替え前に{amount}のビルダー手数料を支払いました。今後OneKeyまたは他の0%ウォレットでの取引はビルダー手数料がかかりません。', '전환 전 {amount}의 빌더 수수료를 지불했습니다. 앞으로 OneKey 또는 기타 0% 지갑을 통한 거래에는 빌더 수수료가 없습니다.', 'Pagaste {amount} en comisiones builder antes de cambiar. Las futuras operaciones vía OneKey u otras billeteras 0% no tendrán comisión builder.', 'Vous avez payé {amount} en frais builder avant de changer. Les trades futurs via OneKey ou d\'autres wallets 0% n\'auront pas de frais builder.', 'Du hast {amount} an Builder-Gebühren bezahlt bevor du gewechselt hast. Zukünftige Trades über OneKey oder andere 0%-Wallets haben keine Builder-Gebühr.', 'Você pagou {amount} em taxas builder antes de mudar. Trades futuros via OneKey ou outras carteiras 0% não terão taxa builder.', 'Вы заплатили {amount} комиссий билдера до переключения. Будущие сделки через OneKey или другие кошельки с 0% не будут иметь комиссию билдера.', 'Geçiş öncesinde {amount} builder ücreti ödediniz. OneKey veya diğer %0 cüzdanlarla yapılacak işlemlerde builder ücreti alınmayacaktır.'),
  ...
  'switch.walletWarningDesc': e('If you keep using a wallet that charges builder fees (like {wallet}), you\'ll still pay their fee on every trade. Switch to a 0% wallet like OneKey or other zero-fee options to stop paying builder fees permanently.','如果你继续使用收取 Builder Fee 的钱包（如 {wallet}），每笔交易仍然会被收费。建议切换到 OneKey 或其他零费率钱包，从根本上杜绝 Builder Fee。','ビルダー手数料を請求するウォレット（{wallet}など）を使い続けると、毎回手数料がかかります。OneKeyなどの0%手数料ウォレットに切り替えましょう。','빌더 수수료를 부과하는 지갑({wallet})을 계속 사용하면 매 거래마다 수수료가 발생합니다. OneKey 또는 기타 0% 수수료 지갑으로 전환하세요.','Si sigues usando una billetera que cobra builder fees (como {wallet}), seguirás pagando. Cambia a OneKey u otra billetera con 0% de comisión.','Si vous continuez avec un wallet qui facture des frais builder (comme {wallet}), vous paierez toujours. Passez à OneKey ou un autre wallet à 0%.','Wenn du weiterhin eine Wallet mit Builder-Gebühren ({wallet}) nutzt, zahlst du weiter. Wechsle zu OneKey oder einer anderen 0%-Wallet.','Se continuar usando uma carteira que cobra builder fees (como {wallet}), continuará pagando. Mude para a OneKey ou outra carteira com 0% de taxa.','Если продолжите использовать кошелёк с комиссией ({wallet}), вы всё равно будете платить. Перейдите на OneKey или другой кошелёк с 0%.','Builder ücreti alan bir cüzdan ({wallet}) kullanmaya devam ederseniz ücret ödemeye devam edersiniz. OneKey veya diğer %0 ücretli cüzdanlara geçin.'),
  'switch.referralValue': e('Save 4% on exchange fees','节省 4% 交易所手续费','取引手数料を4%節約','거래 수수료 4% 절약','Ahorra 4% en comisiones','Économisez 4% sur les frais','4% Handelsgebühren sparen','Economize 4% nas taxas','Экономьте 4% на комиссиях','İşlem ücretlerinde %4 tasarruf'),
  'switch.referralValueDesc': e('Binding a referral code gives you a 4% discount on Hyperliquid taker fees. Everyone pays taker fees — this is free savings you shouldn\'t miss.','绑定推荐码可享受 Hyperliquid 吃单手续费 4% 折扣。所有人都要付吃单费 — 这是白捡的优惠，别错过。','紹介コードを設定するとHyperliquidのテイカー手数料が4%割引されます。全員がテイカー手数料を払います — 見逃せない無料の節約です。','추천 코드를 등록하면 Hyperliquid 테이커 수수료 4% 할인을 받을 수 있습니다. 모든 사용자가 테이커 수수료를 내기에 — 놓칠 수 없는 혜택입니다.','Al vincular un código de referido, obtienes un 4% de descuento en comisiones taker de Hyperliquid. Todos pagan comisiones taker — es un ahorro gratis.','En liant un code de parrainage, vous bénéficiez de 4% de réduction sur les frais taker Hyperliquid. Tout le monde paie les frais taker — c\'est de l\'économie gratuite.','Mit einem Empfehlungscode erhalten Sie 4% Rabatt auf Hyperliquid Taker-Gebühren. Jeder zahlt Taker-Gebühren — kostenlose Ersparnis.','Ao vincular um código de referência, você ganha 4% de desconto nas taxas taker da Hyperliquid. Todos pagam taxas taker — é economia grátis.','Привязка реферального кода даёт 4% скидку на комиссии тейкера Hyperliquid. Все платят комиссии — это бесплатная экономия.','Referans kodu bağlamak Hyperliquid taker ücretlerinde %4 indirim sağlar. Herkes taker ücreti öder — bu kaçırılmaması gereken ücretsiz tasarruf.'),
  ...
  'about.p1': e(
    'Every time you trade on Hyperliquid through a third-party wallet like Phantom, MetaMask, or Rabby, you\'re being charged an extra <b>builder fee</b> — on top of the exchange\'s base fees (0.045% taker / 0.015% maker at VIP 0). Most traders don\'t even notice it.',
    '每次通过 Phantom、MetaMask 或 Rabby 等第三方钱包在 Hyperliquid 上交易时，你都在被额外收取一笔 <b>Builder 费用</b> — 这是在交易所基础费率（VIP 0 时 Taker 0.045% / Maker 0.015%）之上的隐藏成本。大多数交易者根本没有察觉。',
    'Phantom、MetaMask、Rabbyなどのサードパーティウォレットを使ってHyperliquidで取引するたびに、取引所の基本手数料（VIP 0でテイカー0.045%/メイカー0.015%）に加えて<b>ビルダー手数料</b>が上乗せされています。ほとんどのトレーダーはこれに気づいていません。',
    'Phantom, MetaMask, Rabby 같은 서드파티 지갑으로 Hyperliquid에서 거래할 때마다, 거래소 기본 수수료(VIP 0 기준 Taker 0.045% / Maker 0.015%) 위에 <b>빌더 수수료</b>가 추가로 부과됩니다. 대부분의 트레이더는 이를 인지하지 못합니다.',
    'Cada vez que operas en Hyperliquid con billeteras como Phantom, MetaMask o Rabby, te cobran una <b>comisión de builder</b> extra — además de las comisiones base del exchange (0.045% taker / 0.015% maker en VIP 0). La mayoría de los traders ni se enteran.',
    'Chaque fois que vous tradez sur Hyperliquid via un wallet comme Phantom, MetaMask ou Rabby, on vous prélève des <b>frais de builder</b> supplémentaires — en plus des frais de base (0.045% taker / 0.015% maker au VIP 0). La plupart des traders ne s\'en rendent même pas compte.',
    'Jedes Mal, wenn du über ein Drittanbieter-Wallet wie Phantom, MetaMask oder Rabby auf Hyperliquid tradest, zahlst du eine versteckte <b>Builder-Gebühr</b> — zusätzlich zu den Basis-Börsengebühren (0,045% Taker / 0,015% Maker bei VIP 0). Die meisten Trader merken es nicht einmal.',
    'Toda vez que você negocia na Hyperliquid usando carteiras como Phantom, MetaMask ou Rabby, uma <b>taxa de builder</b> extra é cobrada — além das taxas base da exchange (0,045% taker / 0,015% maker no VIP 0). A maioria dos traders nem percebe.',
    'Каждый раз, когда вы торгуете на Hyperliquid через сторонний кошелёк вроде Phantom, MetaMask или Rabby, с вас берут дополнительную <b>комиссию билдера</b> — сверх базовой комиссии биржи (0,045% тейкер / 0,015% мейкер на VIP 0). Большинство трейдеров этого даже не замечают.',
  ...
  'about.p3': e(
    'The good news: some wallets charge nothing. OneKey and other zero-fee wallets have a 0% builder fee. HypeFees helps you see exactly what you\'re paying — and switch to save.',
    '好消息是：有些钱包完全不收费。OneKey 等零费率钱包的 Builder 费率为 0%。HypeFees 让你一目了然看到自己在付多少 — 然后一键切换省钱。',
    '朗報は、手数料ゼロのウォレットもあるということ。OneKeyなどのゼロ手数料ウォレットは0%です。HypeFeesで実際の支払額を確認して、すぐに切り替えて節約しましょう。',
    '좋은 소식은, 수수료를 전혀 안 받는 지갑도 있다는 겁니다. OneKey 등 제로 수수료 지갑의 빌더 수수료는 0%입니다. HypeFees로 현재 지출을 확인하고, 전환해서 절약하세요.',
    'La buena noticia: algunas billeteras no cobran nada. OneKey y otras billeteras con comisión cero tienen 0%. HypeFees te muestra exactamente lo que pagas — y te ayuda a cambiar para ahorrar.',
    'Bonne nouvelle : certains wallets ne facturent rien. OneKey et d\'autres wallets à frais zéro sont à 0%. HypeFees vous montre exactement ce que vous payez — et vous aide à changer pour économiser.',
    'Die gute Nachricht: Manche Wallets verlangen gar nichts. OneKey und andere gebührenfreie Wallets haben 0% Builder-Gebühr. HypeFees zeigt dir genau, was du zahlst — und hilft dir beim Wechsel zum Sparen.',
    'A boa notícia: algumas carteiras não cobram nada. A OneKey e outras carteiras com taxa zero têm 0% de taxa builder. O HypeFees mostra exatamente quanto você paga — e ajuda a mudar para economizar.',
    'Хорошая новость: некоторые кошельки не берут ничего. У OneKey и других кошельков с нулевой комиссией ставка 0%. HypeFees покажет, сколько вы платите на самом деле — и поможет переключиться для экономии.',
    'İyi haber: bazı cüzdanlar hiç ücret almıyor. OneKey ve diğer sıfır ücretli cüzdanların builder ücreti %0. HypeFees tam olarak ne kadar ödediğinizi gösterir — ve tasarruf için geçiş yapmanızı sağlar.',
  ),
  ...
  'how.p2': e(
    'Your effective rate = base fee + builder fee. A VIP 0 taker with a 0.05% builder pays 0.095% total. With a 0% builder, you pay only 0.045% — that\'s 53% less.',
    '你的实际费率 = 基础费用 + Builder 费用。VIP 0 的 Taker 使用 0.05% Builder 时总费率为 0.095%。使用 0% Builder 时只需 0.045% — 直接省了 53%。',
    '実効レート = 基本手数料 + ビルダー手数料。VIP 0のテイカーで0.05%ビルダーなら合計0.095%。0%ビルダーなら0.045%のみ — 53%もお得。',
    '유효 수수료율 = 기본 수수료 + 빌더 수수료. VIP 0 테이커가 0.05% 빌더 사용 시 총 0.095%. 0% 빌더 사용 시 0.045%만 — 53% 절약.',
    'Tu tasa efectiva = base + builder. Un taker VIP 0 con 0.05% builder paga 0.095% en total. Con 0%, pagas solo 0.045% — un 53% menos.',
    'Votre taux effectif = base + builder. Un taker VIP 0 avec 0.05% builder paie 0.095%. Avec 0%, seulement 0.045% — soit 53% de moins.',
    'Dein effektiver Satz = Basis + Builder. Ein VIP 0 Taker mit 0,05% Builder zahlt 0,095%. Mit 0% Builder nur 0,045% — das sind 53% weniger.',
    'Sua taxa efetiva = base + builder. Um taker VIP 0 com builder 0,05% paga 0,095% no total. Com 0%, paga apenas 0,045% — 53% a menos.',
    'Ваша эффективная ставка = базовая + билдер. Тейкер VIP 0 с билдером 0,05% платит 0,095%. С билдером 0% — только 0,045%, то есть на 53% меньше.',
  ...
  'faq.a2': e('OneKey and several other wallets charge 0% builder fees. Check the comparison table above to find the best option for you.','OneKey 和其他几个钱包都提供 0% 费率。查看上方对比表，找到最适合你的选择。','OneKeyをはじめ、複数のウォレットが0%手数料を提供しています。上の比較表であなたに最適な選択を見つけてください。','OneKey를 비롯한 여러 지갑이 0% 빌더 수수료를 제공합니다. 위의 비교표에서 가장 적합한 옵션을 확인하세요.','OneKey y otras billeteras cobran 0%. Mira la tabla de comparación arriba para encontrar la mejor opción.','OneKey et d\'autres wallets facturent 0%. Consultez le tableau comparatif ci-dessus pour trouver le meilleur choix.','OneKey und andere Wallets berechnen 0%. Schau in die Vergleichstabelle oben, um die beste Wahl für dich zu finden.','OneKey e outras carteiras cobram 0%. Confira a tabela de comparação acima para encontrar a melhor opção.','OneKey и другие кошельки берут 0%. Посмотрите таблицу сравнения выше, чтобы найти лучший вариант для себя.','OneKey ve diğer cüzdanlar %0 alır. En iyi seçeneği bulmak için yukarıdaki karşılaştırma tablosuna bakın.'),
